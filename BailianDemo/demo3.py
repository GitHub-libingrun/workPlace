...
def parse_html(address):
    try:
        ...
        lng = dict_s["data"]["geocodes"][0]["location"]["lon"]
        lat = dict_s["data"]["geocodes"][0]["location"]["lat"]
...
def write_data(d):
    with open("error5.txt", "a", encoding="utf-8") as f:
        f.write(d + ";")
def main():
    source = "卡车之家-服务站"
    # brand_url_list = get_all_brand()
    # print(brand_url_list)
    brand_url_list=['https://www.chinatruck.org/service/b3924_area_0','https://www.chinatruck.org/service/b3374_area_0','https://www.chinatruck.org/service/b3389_area_0','https://www.chinatruck.org/service/b3384_area_0','https://www.chinatruck.org/service/b3399_area_0','https://www.chinatruck.org/service/b3387_area_0','https://www.chinatruck.org/service/b3395_area_0','https://www.chinatruck.org/service/b3377_area_0','https://www.chinatruck.org/service/b3404_area_0','https://www.chinatruck.org/service/b3415_area_0','https://www.chinatruck.org/service/b3398_area_0']
                    ...
                    if len(detail_list)>0 and len(detail_list)<16:
                        ...
                        for detail in detail_list:
                            try:
                                ...
                                main_brand=detail.xpath("./a/p[1]//text()")[1].strip() if len(detail.xpath("./a/p[1]//text()"))==2 else ""
                                address=detail.xpath("./a/p[@class='address']/span/text()")[0].strip() if len(detail.xpath("./a/p[@class='address']/span/text()")) else ""
                                city_linshi=""
                                if "|" in address:
                                    address = address.split("|")[1]
                                    city_linshi=address.split("|")[0]
                                else:
                                    address=key
                                is_in="0"
                                if address:
                                ...
                                tela=detail.xpath("./a/p/span[@class='tel']/text()")[0].strip() if detail.xpath("./a/p/span[@class='tel']/text()") else ""
                                province=key
                                ...
                                if p_list and province in p_list[0]:
                                    ...
                                    p_list = parse_html(key+city_linshi+address)
                                    if p_list and province in p_list[0]:
                                        postcode = post_code_dicts.get(p_list[4], "")
                                        print(
                                            "insert into tb_kache_data(name,address,postcode,tel,source,source_url,p_name, city_name, ad_name, p_code, city_code, ad_code, lng, lat, geo_hase,main_brand) values('%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s');" %
                                            (name, address, postcode, tel, source, d_url, p_list[0], p_list[1],
                                             p_list[2], p_list[3], p_list[4], p_list[5], p_list[6], p_list[7],
                                             p_list[8], main_brand))
                                        sqlstr = "insert into tb_kache_data(name,address,postcode,tel,source,source_url,p_name, city_name, ad_name, p_code, city_code, ad_code, lng, lat, geo_hase,main_brand,is_in) values('%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s');" % \
                                                 (name, address, postcode, tel, source, d_url, p_list[0], p_list[1],
                                                  p_list[2], p_list[3], p_list[4], p_list[5], p_list[6], p_list[7],
                                                  p_list[8], main_brand, is_in)
                                        run_sql(sqlstr)
